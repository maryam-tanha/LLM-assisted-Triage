...
% Parent → Evaluator (when concluding)
% Route outside right edge of processing layer, drop into the OUTPUT LAYER padding, then left to eval.north
\coordinate (concludeRight) at ($(proclayer.east) + (18pt, 0)$);
\coordinate (concludeMidY) at ($(eval.north) + (0, 25pt)$);
\draw[arrgr, very thick, rounded corners=6pt]
    (parent.east)
    -- node[above, sublabel, pos=0.8]{conclude}
    (parent.east -| concludeRight)
    -- (concludeRight |- concludeMidY)
    -- (eval.north |- concludeMidY)
    -- (eval.north);
