...
We evaluate on a four-service mail application deployed on an Amazon Web Services (AWS) Elastic Compute Cloud (EC2) instance,
...
agent a Standard Operating Procedure modeled on real-world software team workflows, which constrains
output format and reduces coordination errors. Both works converge on the same
...
anomalies, forcing the planner to explore multiple diagnostic paths rather than commit to a
flat task list upfront.
...
Ashraf and Talavera~\cite{ashraf2025autonomous} observe that coordination failures
increase when agents lack clear role separation; conflicting decisions were most frequent
in tasks where multiple agents operated over the same problem scope.
...
Looking ahead to Phase~2, the AWS Model Context Protocol (MCP) architecture~\cite{aws2025mcp_sre} shows how MCP servers let agents discover tools dynamically without bespoke Application Programming Interface (API) wrappers.
...
We evaluated the framework on a four-service mail application
...
with Apache and PHP (webmail front-end), and PostgreSQL (session and
address-book storage). This target was chosen over the simpler voting
...
LLM reasoning is more reliable when grounded in a declared service
topology than when left to infer structure from raw logs; iterative
...
orchestrator prompts) without writing Python. On our four-service
mail application, six fault injections spanning five infrastructure
